...
class CombineVideoAudio:
    ...
    def _save_audio_wav(self, final_audio_path, waveform, sample_rate):
        """Save a ComfyUI AUDIO waveform as PCM16 WAV without torchaudio/TorchCodec."""
        ...
        # Expected torchaudio/Comfy shape is channels x samples. Convert to samples x channels.
        if waveform.ndim == 1:
            audio_np = waveform[:, None]
            ...
            raise ValueError(f"Unsupported AUDIO waveform shape: {waveform.shape}")

        pcm16 = (np.clip(audio_np, -1, 1) * 32767.0).astype(np.int16)
        with wave.open(final_audio_path, "wb") as wav_file:
    ...
    def combine_audio_video(self, IMAGES=None, AUDIO=None, audio_path="", video_path="", fps=30.0):
        ...
        try:
            ...
            if audio_path and os.path.exists(audio_path):
                ...
                # Save WAV without torchaudio.save/TorchCodec. Recent torchaudio
                # requires torchcodec for saving under Python 3.13, but ComfyUI
                # AUDIO tensors are simple PCM-friendly waveforms.
                self._save_audio_wav(final_audio_path, waveform, sample_rate)
            else:
